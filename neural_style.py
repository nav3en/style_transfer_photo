...
def parse_args():
    ...
    parser.add_argument('--laplacian_dir', type=str, default='./video_input/laplacian',
    ...
    parser.add_argument('--lambda_val', type=float,
                        default=10 ** 4,
                        help='Weight for the photorealism regularization. (default: %(default)s)')
...
def stylize(content_img, style_imgs, init_img, frame=None, laplacian_mat=None):
    with tf.device(args.device), tf.Session() as sess:
        ...
        if args.video and frame > 1:
            lamda = args.lambda_val
            L_total += lamda * L_photorealism_reg
...
def render_video():
    # Logic to get laplacian mat paths list from directory
    laplacian_dir = args.laplacian_dir
    laplacians_path = list()
    for file in os.listdir(laplacian_dir):
        if file.endswith(".npz"):
            laplacians_path.append(os.path.join(laplacian_dir, file))

    for frame in range(args.start_frame, args.end_frame + 1):
        laplacian_mat = laplacians_path[frame - 1]
        with tf.Graph().as_default():
            ...
            if frame == 1:
                ...
                tick = time.time()
                stylize(content_frame, style_imgs, init_img, frame, laplacian_mat)
                tock = time.time()
